...
PROTOTYPE: name and arguments of the test function. Arguments should be contained within parenthesis and separated py commas.
	Example: for function "testFn" with arguments "filename", "point", and "vertices", PROTOTYPE would be "testFn(filename,point,vertices)"
	Available arguments:
		Mesh mode: the following arguments can be used when MODE is set to "mesh"
		...
		Voxel mode: the following arguments can be used when MODE is set to "voxel"
			filename (string): contains the absolute path and filename of the .obj file which stores the current test data
